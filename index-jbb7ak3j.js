...
__export(exports_src2, {
  yellow: () => yellow,
  wrapWithDelegates: () => wrapWithDelegates,
  white: () => white,
  vstyles: () => vstyles,
  ...
  resolveCoreSlot: () => resolveCoreSlot,
  renderFontToFrameBuffer: () => renderFontToFrameBuffer,
  registerEnvVar: () => registerEnvVar,
  registerCorePlugin: () => registerCorePlugin,
  red: () => red,
  ...
  parseMeasureMode: () => parseMeasureMode,
  parseLogLevel: () => parseLogLevel,
  parseKeypress: () => parseKeypress,
  parseJustify: () => parseJustify,
  parseGutter: () => parseGutter,
  ...
  isVNode: () => isVNode,
  isTextNodeRenderable: () => isTextNodeRenderable,
  isStyledText: () => isStyledText,
  isRenderable: () => isRenderable,
  isEditBufferRenderable: () => isEditBufferRenderable,
  ...
  generateEnvMarkdown: () => generateEnvMarkdown,
  ...
  coordinateToCharacterIndex: () => coordinateToCharacterIndex,
  convertThemeToStyles: () => convertThemeToStyles,
  ...
  Timeline: () => Timeline,
  TimeToFirstDrawRenderable: () => TimeToFirstDrawRenderable,
  TextareaRenderable: () => TextareaRenderable,
  TextTableRenderable: () => TextTableRenderable,
  TextRenderable: () => TextRenderable,
  TextNodeRenderable: () => TextNodeRenderable,
  TextBufferView: () => TextBufferView,
  TextBufferRenderable: () => TextBufferRenderable,
  ...
  LogLevel: () => LogLevel,
  LinearScrollAccel: () => LinearScrollAccel,
  LineNumberRenderable: () => LineNumberRenderable,
  LayoutEvents: () => LayoutEvents,
  KeyHandler: () => KeyHandler,
  KeyEvent: () => KeyEvent,
  InternalKeyHandler: () => InternalKeyHandler,
  ...
  EditBufferRenderableEvents: () => EditBufferRenderableEvents,
  EditBufferRenderable: () => EditBufferRenderable,
...
// ../../node_modules/.bun/diff@9.0.0/node_modules/diff/libesm/util/string.js
...
// ../../node_modules/.bun/diff@9.0.0/node_modules/diff/libesm/diff/word.js
...
class DiffRenderable extends Renderable {
  ...
  _leftDiffHighlightRanges = null;
  _rightDiffHighlightRanges = null;
  _leftDiffOnHighlight = null;
  _rightDiffOnHighlight = null;
  leftSide = null;
  rightSide = null;
  leftSideAdded = false;
...
`);
    ...
    if (shouldDoAlignment) {
      ...
      const leftLineInfoReady = leftCodeRenderable.lineCount === leftLogicalLines.length && leftSources.length > 0 && leftSources.every((line) => line >= 0 && line < leftLogicalLines.length);
      const rightLineInfoReady = rightCodeRenderable.lineCount === rightLogicalLines.length && rightSources.length > 0 && rightSources.every((line) => line >= 0 && line < rightLogicalLines.length);
...
`);
  ...
  buildVisualCounts(sources, lines, lineInfoReady, sideWidth) {
  ...
  updateDiffHighlighting(leftCodeRenderable, rightCodeRenderable, finalLeftLines, finalRightLines) {
    const leftHighlightRanges = [];
    const rightHighlightRanges = [];
    ...
    if (!this._leftDiffOnHighlight) {
      this._leftDiffOnHighlight = (highlights) => {
        if (!this._leftDiffHighlightRanges)
          return highlights;
        return [
          ...highlights,
          ...this._leftDiffHighlightRanges.map((range) => [range[0], range[1], "diff-removed-highlight"])
        ];
      };
    }
    if (!this._rightDiffOnHighlight) {
      this._rightDiffOnHighlight = (highlights) => {
        if (!this._rightDiffHighlightRanges)
          return highlights;
        return [
          ...highlights,
          ...this._rightDiffHighlightRanges.map((range) => [range[0], range[1], "diff-added-highlight"])
        ];
      };
    }
  ...
  clearDiffHighlighting(leftCodeRenderable, rightCodeRenderable) {
  ...
  get highlightAddedBg() {
    return this._highlightAddedBg;
  }
  set highlightAddedBg(value) {
    const parsed = value ? parseColor(value) : null;
  ...
  get highlightRemovedBg() {
    return this._highlightRemovedBg;
  }
  set highlightRemovedBg(value) {
    const parsed = value ? parseColor(value) : null;
...
function changedTokenRanges(left, right) {
  ...
  let leftOffset = 0;
  let rightOffset = 0;
  for (const change of diffWordsWithSpace(left, right, { maxEditLength: 1e4 }) ?? []) {
    if (change.removed) {
...
//# debugId=EAEC3B4F09213DA964756E2164756E21
//# sourceMappingURL=index-jbb7ak3j.js.map
