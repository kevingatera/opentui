import type { CommandDefinition, CommandContext, CommandRecord, ActionMap, ParsedCommand } from "../types.js"
...
export function registerExCommands(actionMap: ActionMap, commands: ExCommand[]): () => void {
  ...
  const offResolver = actionMap.appendCommandResolver((input, ctx) => {
    if (!input.startsWith(":")) {
      return undefined
    }
