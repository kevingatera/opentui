...
// Last generated: 2026-05-11T02:47:04.356Z
...
import diff_highlights from "./assets/diff/highlights.scm" with { type: "file" }
import diff_language from "./assets/diff/tree-sitter-diff.wasm" with { type: "file" }
  ...
  if (!_cachedParsers) {
    _cachedParsers = [
      ...
      },
      {
        filetype: "diff",
        queries: {
          highlights: [resolve(dirname(fileURLToPath(import.meta.url)), diff_highlights)],
        },
        wasm: resolve(dirname(fileURLToPath(import.meta.url)), diff_language),
      },
      {
        filetype: "zig",
        queries: {
