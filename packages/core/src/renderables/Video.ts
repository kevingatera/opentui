...
const VIDEO_CPU_OVERLOAD_RATIO = 1
const VIDEO_QUALITY_RECOVERY_RATIO = 0.9
...
export class VideoRenderable extends Renderable {
  ...
  constructor(ctx: RenderContext, options: VideoRenderableOptions) {
    ...
    this.avSyncOffsetValue = options.avSyncOffsetMs ?? 0
  ...
  public get avSyncOffsetMs(): number {
    return this.avSyncOffsetValue
  }

  public set avSyncOffsetMs(value: number) {
  ...
  private validateAvSyncOffset(value: number): void {
    if (!Number.isFinite(value) || !Number.isSafeInteger(Math.round(value * 1000))) {
      throw new RangeError("avSyncOffsetMs must resolve to a safe number of microseconds")
    }
  }
