...
| `resolveBindingSections(config)` | Groups flat dot-delimited command config into binding sections plus `get(section, cmd)`, `pick(...)` and `omit(...)` helpers |
...
`resolveBindingSections()` is an opinionated config-to-keymap transformation for flat command config. Keys must be dot-delimited command names; the first segment becomes the section, and the full key becomes `binding.cmd`. Values can be `false`, `"none"`, a key, a full binding object, or arrays of keys/binding objects. `false`, `"none"`, and empty arrays emit no bindings.
...
const resolved = resolveBindingSections(
  ...
  },
  {
    bindingDefaults({ section, binding }) {
      ...
      return { group: sectionGroups[section as keyof typeof sectionGroups] }
    },
  },
)
...
resolved.pick("prompt", ["prompt.history.previous", "prompt.history.next"])
resolved.omit("app", ["app.exit"])
```

The resolved object exposes three views:

| API                       | Description                                                                                                      |
| ------------------------- | ---------------------------------------------------------------------------------------------------------------- |
| `sections[section]`       | All enabled bindings for the derived section, preserving config order                                            |
| `get(section, command)`   | Bindings for one command, or `undefined` when the section or command is missing/disabled                         |
| `pick(section, commands)` | Bindings for the listed commands in caller command order; missing or disabled commands are skipped               |
| `omit(section, commands)` | All section bindings except matching string-command bindings, preserving section order; missing commands ignored |

Use `bindingDefaults` to add app-level fields to every emitted binding without mutating the original config. Defaults are spread before the resolved binding, so explicit binding fields win. `pick()` and `omit()` use exact command names and do not trim. They are meant for hard-coded application command lists. Use them when one derived config section feeds multiple activation layers.
