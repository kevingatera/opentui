...
keymap.registerLayer({ bindings: bindings.gather("app", ["show_palette", "exit_app"]) })
bindings.get("exit_app")
bindings.pick("app", ["exit_app"])
bindings.omit("app", ["show_palette"])
...
| `get(command)`           | Bindings for one command, or `undefined` when the command is missing/disabled                            |
...
Use `bindingDefaults` to add app-level fields to every emitted binding without mutating the original config. Defaults are spread before the resolved binding, so explicit binding fields win. `gather()`, `pick()` and `omit()` use exact command names and do not trim. They are meant for hard-coded application command lists.
