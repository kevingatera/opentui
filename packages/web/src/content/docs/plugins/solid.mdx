...
This is a Bun drop-in that installs runtime transform support so external TS/TSX plugin modules use the same runtime instances as the host app (`@opentui/solid`, `@opentui/core`, `@opentui/three`, `@opentui/core/testing`, `solid-js`, and `solid-js/store`). If a plugin imports `@opentui/three`, install `@opentui/three` in the host app so the runtime loader can provide it.
